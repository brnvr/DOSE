function game_set_music_track(sound_id, gain=1, fade_time=4000) {
	...
	if (obj_game_control.music_track != -1) {
		audio_sound_gain(obj_game_control.music_track, 0, fade_time);
		
		if (obj_game_control.music_track_previous != -1 && time_source_get_state(obj_game_control.ts_stop_music_track) == time_source_state_active) {
