function pawpr_update(ui) {
	var n_elements, mouse_x_new, mouse_y_new, has_mouse_moved;
	
	n_elements = array_length(ui[pawpr_attr.elements]);

	mouse_x_new = pawpr_mouse_x(ui);
	mouse_y_new = pawpr_mouse_y(ui);

	has_mouse_moved = (mouse_x_new != ui[pawpr_attr.mouse_x] || mouse_y_new != ui[pawpr_attr.mouse_y]);
	...
	for (var i = 0; i < n_elements; i++) {
		var element, element_n_events;
		
		element = ui[pawpr_attr.elements][i];
		element_n_events = array_length(element[pawpr_attr.events]);
		...
		if (has_mouse_moved) {
			...
			is_mouse_over = pawpr_is_mouse_over(element);
		...
		for (var j = 0; j < element_n_events; j++) {
			var event_current, event_current_trigger;
			
			event_current = element[pawpr_attr.events][j];
			if (is_undefined(event_current)) break;
			
			event_current_trigger = pawpr_event_get_trigger(event_current[pawpr_dict.key]);
