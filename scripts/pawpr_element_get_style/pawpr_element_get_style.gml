function pawpr_element_get_style(element, property, default_value=undefined, use_selectors=true) {
	...
	if (use_selectors && !is_undefined(element[pawpr_attr.ui])) {
		...
		selectors = element[pawpr_attr.style][pawpr.selectors];
		...
		if (!is_undefined(selectors)) {
			...
			for (var i = 0; i < n_selectors; i++) {
				...
				var selector_index, selector_predicate;
				
				selector_index = selectors[i][pawpr_dict.key];				
				selector_predicate = pawpr_selector_get_predicate(selector_index);
				...
				if (selector_predicate(element)) {
					...
					selector_style = selectors[i][pawpr_dict.value];
