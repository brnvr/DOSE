function door_random_on_interact() {
	static resolve_hint = function() {
		var resolve_hint = true
		
		if (!is_undefined(resolve_hint)) {
			obj_player.resolve_hint(hint_types.go_back_one_room, id)	
		}
		
		if (revert_hint) {
			if (obj_control.current_area != revert_hint_opposite_area) {
				obj_player.revert_hint(hint_types.go_back_one_room, id)
			}	
		}	
	}
		...
		if (dest_area == noone) {
			...
			if (current_area.object_index == obj_room) {
				...
				if (take_chance(prob_room)) {
					...
					dest_area = door_make_corridor(dir)
				}
			}
