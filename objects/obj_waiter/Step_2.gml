if (obj_game_control.current_area.object_index == obj_custom_area) {
	...
	if (obj_player.door_passing != noone) {
		...
		if (obj_game_control.current_area != noone) {
			if (array_length(obj_game_control.current_area.doors) == 0) {
			...
			if (go_to_random_door) {
				door_destination = array_choose(obj_game_control.current_area.doors);
			} else {
	...
	if (is_moving) {
		if (is_undefined(coords_destination)) {
			if (current_area == obj_game_control.current_area) {
...
if (!is_transforming) {
	visible = current_area == obj_game_control.current_area	
}
